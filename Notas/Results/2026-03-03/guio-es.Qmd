...
str(P01923)
nombre_largo_ejercicio3 <- P01923[['metadata']][['sequence']]
nombre_largo_ejercicio3
```

Sí que la podemos obtener, ejecutando este chunk
...
```{r}
taxonomy <- fromJSON(paste0(API, 'taxonomy'))[['taxa']][['uniprot']]
taxonomy[[1]]

ejercicio4 <- fromJSON(paste0(API, 'taxonomy/protein'))[['taxa']][['uniprot']]
ejercicio4[[1]]

resultado <-  taxonomy - ejercicio4
resultado[[1]]
```

Habrá 1387422 taxones para los que no hay proteinas descritas. El motivo probablemente sea que entre las proteínas no revisadas han dejado de añadir las de taxones muy parecidos a otros de los que sí se conocen a ciencia cierta sus proteínas.

```{r}
ProtTot <- fromJSON(paste0(API, 'protein'))[['proteins']][['uniprot']]
ProtTot

ProtStruc <- fromJSON(paste0(API, 'protein/structure'))[['proteins']][['uniprot']]
ProtStruc[[1]]

Proporción <- ProtStruc / ProtTot 
Proporción
```
...
flatten(pfam00019[['results']][['metadata']])
...
```{r}
vert <- fromJSON(paste0(API, 'entry/pfam/taxonomy/uniprot/7742'))
names(vert)
vert[['count']]
names(vert[['results']])

# La función "flaten()" corrige el anidamiento de marcos de datos,
flatten(vert[['results']][['metadata']])
...
```{r}
ejercicio6 <- fromJSON(paste0(API, 'protein/taxonomy/uniprot/8493/entry/pfam/PF01530'))
ejercicio6

```
:::

Hay 15 proteínas que cumplen esa condición
